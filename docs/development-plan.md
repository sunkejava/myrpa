...
- [ ] 高风险提交幂等、外部状态核验、Checkpoint 恢复与自动恢复测试（Step 事件记录和盲重试阻断已具备；管理员独立核验失败执行后可凭外部单号结案或确认未提交再重试，结论写入原 Execution 日志，CI 覆盖重复提交后结案、提交前故障后手动重试及次数耗尽；真实外部系统自动对账和安全断点续跑尚待完成）。
