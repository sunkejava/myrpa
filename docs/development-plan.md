...
### Phase 8
- [ ] Node / NodePool / Capability / WorkerSlot 完整管理 UI（节点管理页支持审核、排空、吊销和 WorkerSlot 启停；节点池页支持创建、重命名、停启、删除和分配；能力页支持禁用/启用已上报能力，Agent 重注册不会取消管理员禁用；完整详情与运行指标待完成）。
- [ ] 调度监控 / ExecutionDispatch 时间线（已记录自动及人工派发、离线回退，并提供归属隔离的执行事件与节点租约时间线；全局调度监控大盘待完成）
