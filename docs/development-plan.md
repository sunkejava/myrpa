...
### Phase 2
- [ ] Designer 完整 API（创建、版本查询、发布、停用及模板草稿展示已接入页面；逐 Step 编辑、排序/删除，以及 Condition 的 then/else、Loop 和 SubWorkflow 的嵌套步骤表单已支持；在线调试仍待完成）
...
### Phase 7
- [ ] 厂商 UKey SDK / PIN / 签名 Provider（Windows 当前用户证书存储可执行 RSA/ECDSA SHA-256 摘要签名；节点 `UKeySign` 在人工确认后读取摘要、签名并填回页面；厂商专有 SDK、硬件 PIN 会话及真实设备验收仍待完成）
...
### Phase 8
- [ ] Node / NodePool / Capability / WorkerSlot 完整管理 UI（节点管理页支持审核、排空、吊销和 WorkerSlot 启停；节点池页支持创建、重命名、停启、删除和分配；能力页支持禁用/启用已上报能力，Agent 重注册不会取消管理员禁用；节点详情展示能力、槽位、执行统计和最近执行；心跳已上报 Agent 进程 CPU、进程内存与可用槽位快照；历史指标、告警与资源趋势待完成）。
...
### Phase 9
- [ ] 更多城市 / 业务系统 Adapter（NodeAgent 已支持按本地配置加载目标站点页面地址和语义选择器、启动校验并自动声明能力；真实平台定位符、授权测试和业务回执验收仍待完成）
