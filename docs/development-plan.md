...
- [ ] 完整 Publish / Disable 策略（停用后阻断新任务及待派发任务，通知在线 NodeAgent 协作取消，节点重连时补发取消并阻止人工恢复，StepStarted 再次检查停用状态；取消记录失败供高风险任务核验，CI 覆盖运行中 Wait 和人工等待阶段停用且未进入外部提交步骤。节点进程退出后丢失会话与正在进行的外部副作用仍不能保证立即中止，待完成生产级安全停机）。
