...
| `UKeySign` | `certificateThumbprint`、`digestSelector`、`signatureSelector` | 必须设置根级 `requiresApproval:true` 和 `executionRequirement.requiredCapabilities` 中的 `Certificate:证书指纹`；逐节点编辑时设计器会自动同步审批及证书能力。用户确认后从网页读取 Base64 编码的 32 字节 SHA-256 摘要，以 Windows 节点证书私钥签名并填回页面；不自动提交。依赖驱动支持交互式授权，尚需目标设备验收。 |
