...
3. 逐节点编辑 ID、按节点类型显示的页面地址/选择器/输入值/文件路径等字段、配置 JSON、超时、只读重试次数和动作权限；节点可上下移动或删除。条件分支与嵌套步骤可在节点配置 JSON 中编辑；高级 JSON 编辑用于完整参数 Schema 和执行环境限制。先更新系统地址和实际选择器，再创建并发布新版本。
