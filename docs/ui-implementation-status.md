...
| 页面与 API 组件化 | 外壳布局、会话 Store、认证/审计/用量/任务/规划/资源/权限 API、主题 composable、查询/表格组件分文件；审计、用量、账户、节点能力、节点池、权限记录和资源地区复用表格，审计和用量复用查询表单。 | 进行中：其他管理页面的 API/表单/表格待迁移 |
...
| CRUD 常用组件 | `FormDialog`、`DetailDrawer`、`ConfirmAction` 已独立实现并接入节点池重命名/删除、资源创建、权限授权和节点详情；任务详情与列表分开显示；账户、角色、资源已有真实接口。 | 进行中：其他管理页面的统一迁移及键盘焦点完整性验收待完成 |
