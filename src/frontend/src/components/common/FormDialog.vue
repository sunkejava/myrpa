...
<template>
  <div v-if="open" class="dialog-backdrop" @click.self="emit('close')">
    <section ref="panel" class="form-dialog panel" role="dialog" aria-modal="true" :aria-labelledby="titleId" tabindex="-1" @keydown.esc="emit('close')">
      ...
      <form @submit.prevent="emit('submit')"><slot /><div class="actions"><button type="button" class="action-btn" @click="emit('close')">取消</button><button type="submit" class="action-btn primary" :disabled="busy">{{ submitLabel || '保存' }}</button></div></form>
    </section>
  </div>
</template>
