...
<template>
  <section class="panel form-panel">
    ...
    <form @submit.prevent="assignRole"><h3>分配角色</h3><div class="resource-grid"><label>账户<select :value="selectedUser" required @change="selectUser(($event.target as HTMLSelectElement).value)"><option value="">选择账户</option><option v-for="user in users" :key="user.id" :value="user.id">{{ user.displayName }} ({{ user.userName }})</option></select></label><label>角色<select v-model="roleId" required><option value="">选择角色</option><option v-for="role in roles.filter(x => x.enabled && !assignedRoles.some(assigned => assigned.id === x.id))" :key="role.id" :value="role.id">{{ role.displayName }}</option></select></label></div>
