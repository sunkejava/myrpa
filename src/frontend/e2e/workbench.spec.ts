...
test('login, resource setup, natural language planning and task submission', async ({ page, request, isMobile }) => {
  ...
  await page.getByRole('button', { name: '调度监控' }).click()
  await expect(page.getByText('当前任务与资源')).toBeVisible()
  await expect(page.getByText('最近 30 次执行')).toBeVisible()
  ...
  await page.getByRole('dialog').getByRole('textbox', { name: '编码', exact: true }).fill('TST-CN')
  await page.getByRole('dialog').getByRole('textbox', { name: '名称', exact: true }).fill('测试国家')
  ...
  await page.getByRole('dialog').getByRole('textbox', { name: '编码', exact: true }).fill('SD')
  await page.getByRole('dialog').getByRole('textbox', { name: '名称', exact: true }).fill('山东省')
  await page.getByRole('button', { name: '创建', exact: true }).click()
  await expect(page.getByRole('option', { name: /山东省/ })).toBeAttached()
  await page.locator('.resource-grid select').nth(1).selectOption('')
  await page.locator('.resource-grid select').nth(2).selectOption(city.id)
  ...
  await page.getByRole('dialog').getByLabel('登录名').fill(uiUserName)
  await page.getByRole('dialog').getByLabel('显示名称').fill('页面执行员')
  await page.getByRole('dialog').getByLabel('初始密码').fill('PageRunnerPassword123!')
  await page.getByRole('dialog').getByRole('button', { name: '创建账户' }).click()
  await expect(page.getByRole('status')).toContainText('账户已创建')
  await page.getByRole('row').filter({ hasText: uiUserName }).getByRole('button', { name: '管理角色' }).click()
  await page.getByRole('dialog').getByRole('combobox', { name: '选择角色' }).selectOption(role.id)
  ...
  await expect(page.getByRole('row').filter({ hasText: 'Runner' })).toBeVisible()
  ...
  await page.getByRole('dialog').getByLabel('功能').selectOption(businessFunction.id)
