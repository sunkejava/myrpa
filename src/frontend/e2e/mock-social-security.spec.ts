...
test('青岛模拟社保登录、增员、重复申报、减员及校验', async ({ page, request, isMobile }) => {
  ...
  await expect(page.locator('[data-result=success]')).toContainText('增员申报成功')
  await page.getByRole('link', { name: '返回' }).click()
  await page.locator('[name=employeeName]').fill('另一人员')
  await page.locator('[name=operation]').selectOption('add')
  await page.locator('[name=idNumber]').fill(employee.idNumber)
  await page.locator('[name=submissionId]').fill('mock-add-replay-001')
  await page.getByRole('button', { name: '提交申报' }).click()
  await expect(page.locator('[data-result=error]')).toContainText('业务请求号已用于另一笔申报')
  await page.getByRole('link', { name: '返回' }).click()
  await submit('add')
