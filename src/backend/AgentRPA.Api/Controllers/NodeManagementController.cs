...
public sealed class NodeManagementController(AgentRpaDbContext db) : ControllerBase
{
    ...
    {
        var pool = await db.NodePools.FindAsync([id], ct);
        ...
        // 当前领域模型尚未开放修改方法，这里通过 EF Core 跟踪实体更新私有属性。
        db.Entry(pool).Property(x => x.Name).CurrentValue = request.Name.Trim();
        db.Entry(pool).Property(x => x.Description).CurrentValue = request.Description?.Trim();
        db.Entry(pool).Property(x => x.Enabled).CurrentValue = request.Enabled;
        await db.SaveChangesAsync(ct);
        return Ok(new { pool.Id, pool.Name, pool.Description, pool.Enabled });
    }
