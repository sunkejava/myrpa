...
{
    [HttpGet]
    public async Task<IActionResult> List(CancellationToken ct) => Ok(await db.Tasks.AsNoTracking().OrderByDescending(x => x.Id).Select(x => new { x.Id, x.Name, x.WorkflowId, x.WorkflowVersion, x.MaxRetries, Status = x.Status.ToString() }).ToListAsync(ct));
    ...
    public async Task<IActionResult> Import(Guid id, IFormFile file, CancellationToken ct)
    {
        ...
        var task = await db.Tasks.Include(x => x.Items).SingleOrDefaultAsync(x => x.Id == id, ct);
    ...
    public async Task<IActionResult> Queue(Guid id, CancellationToken ct) { var task = await db.Tasks.FindAsync([id], ct); if (task is null) return NotFound(); task.Queue(); await db.SaveChangesAsync(ct); return Ok(); }
