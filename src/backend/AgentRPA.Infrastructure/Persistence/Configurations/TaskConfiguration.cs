...
public sealed class TaskConfiguration : IEntityTypeConfiguration<RpaTask>, IEntityTypeConfiguration<TaskItem>, IEntityTypeConfiguration<Execution>
{
    ...
    void IEntityTypeConfiguration<Execution>.Configure(EntityTypeBuilder<Execution> b) { b.ToTable("executions"); b.HasKey(x => x.Id); b.Property(x => x.DispatchKey).HasMaxLength(128).IsRequired(); b.HasIndex(x => x.DispatchKey).IsUnique(); b.HasIndex(x => x.TaskItemId); b.HasIndex(x => x.NodeId); b.HasIndex(x => x.Status); }
}
