...
{
    public Task<AgentPlanResult> PlanAsync(string instruction, CancellationToken cancellationToken) =>
        PlanAsync(instruction, null, cancellationToken);
    ...
    private async Task<Resolution?> ResolveWithLlmAsync(string text, Guid? subjectId, IReadOnlyList<AgentCityResource> cities, IReadOnlyList<AgentSystemResource> systems, IReadOnlyList<AgentFunctionResource> functions, CancellationToken cancellationToken)
    {
        ...
        if (subjectId.HasValue) await llmUsageRecorder.RecordAsync(subjectId.Value, response, cancellationToken: cancellationToken);
