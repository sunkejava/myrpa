    ...
    ILlmUsageRecorder llmUsageRecorder)
{
    ...
    private async Task<Resolution?> ResolveWithLlmAsync(string text, Guid? subjectId, IReadOnlyList<AgentCityResource> cities, IReadOnlyList<AgentSystemResource> systems, IReadOnlyList<AgentFunctionResource> functions, CancellationToken cancellationToken)
    {
        ...
        {
            var model = TryResolveConfiguredModel();
            await llmUsageRecorder.RecordAsync(subjectId.Value, llmProvider.ProviderId, model, response, cancellationToken: cancellationToken);
        }
        ...
        try
        {
            ...
            if (root.TryGetProperty("parameters", out var parameterElement) && parameterElement.ValueKind == JsonValueKind.Object)
                foreach (var property in parameterElement.EnumerateObject()) parameters[property.Name] = property.Value.Clone();
    ...
    private static string TryResolveConfiguredModel() => "configured";
    private static Resolution ResolveDeterministically(string text, IReadOnlyList<AgentCityResource> cities, IReadOnlyList<AgentSystemResource> systems, IReadOnlyList<AgentFunctionResource> functions)
    {
        ...
        if (cityMatches.Length != 1) return Resolution.Failed(cityMatches.Length == 0 ? "无法从指令识别城市。" : "指令匹配到多个城市，请明确城市。");
        ...
        if (systemMatches.Length != 1) return Resolution.Failed(systemMatches.Length == 0 ? "无法识别业务系统。" : "指令匹配到多个业务系统，请明确系统。");
        ...
        if (functionMatches.Length != 1) return Resolution.Failed(functionMatches.Length == 0 ? "无法识别业务功能。" : "指令匹配到多个业务功能，请明确功能。");
    ...
    private static string ExtractJson(string content)
    {
        ...
        if (value.StartsWith("```") && value.EndsWith("```")) { var firstLine = value.IndexOf('\n'); value = firstLine >= 0 ? value[(firstLine + 1)..^3].Trim() : value.Trim('`'); }
        var start = value.IndexOf('{'); var end = value.LastIndexOf('}'); return start >= 0 && end > start ? value[start..(end + 1)] : value;
    }
